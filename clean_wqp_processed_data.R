...
temp <- out.file %>%
    filter(if_any(everything(), ~str_detect(tolower(.), "nars_wqx-nla")))
out.file <- out.file %>% filter(!sample_id %in% temp$sample_id)


station <- read_csv("station.csv") %>% 
    select(OrganizationIdentifier,MonitoringLocationIdentifier,MonitoringLocationName,LatitudeMeasure,LongitudeMeasure) %>% 
    mutate(LatitudeMeasure = as.character(LatitudeMeasure),
           LongitudeMeasure = as.character(LongitudeMeasure)) %>% 
    mutate(LatitudeMeasure =  sub("0+$", "", as.character(LatitudeMeasure)),
           LongitudeMeasure =  sub("0+$", "", as.character(LongitudeMeasure))) %>% 
    mutate(lat_char =  nchar(gsub(".*\\.|^[^.]+$", "", as.character(LatitudeMeasure))),
           lon_char = nchar(gsub(".*\\.|^[^.]+$", "", as.character(LongitudeMeasure))))

station <- station %>% filter(lat_char <=2) %>% 
    filter(lon_char <=2)


out.file <- out.file %>% filter(!source_sample_siteid %in% station$MonitoringLocationIdentifier)
...
dat <- readRDS("initial_process.rds")
...
unique(temp$source_labmethod_usgspcode)
...
dat <- dat %>% mutate(source_methodqualifier = ifelse(parameter_name=="Chlorophyll a" & source_labmethod_name == "STANDARD METHODS 10200(2)(B) - CHLOROPHYLL A, PHAEOPHYTIN CORRECTION METHOD","corrected",source_methodqualifier))
dat <- dat %>% mutate(source_methodqualifier = ifelse(parameter_name=="Chlorophyll a" & source_labmethod_name == "Chlorophyll a, corrected for pheophytin","corrected",source_methodqualifier))


#deal with usgs pcodes
dat <- dat %>% mutate(source_methodqualifier = ifelse(is.na(source_methodqualifier) & source_labmethod_usgspcode==32209,"corrected",source_methodqualifier))
dat <- dat %>% mutate(source_methodqualifier = ifelse(is.na(source_methodqualifier) & source_labmethod_usgspcode==70953,"uncorrected",source_methodqualifier))
dat <- dat %>% mutate(source_methodqualifier = ifelse(is.na(source_methodqualifier) & source_labmethod_usgspcode==32217,"uncorrected",source_methodqualifier))
dat <- dat %>% mutate(source_methodqualifier = ifelse(is.na(source_methodqualifier) & source_labmethod_usgspcode==32230,"uncorrected",source_methodqualifier))
dat <- dat %>% mutate(source_methodqualifier = ifelse(is.na(source_methodqualifier) & source_labmethod_usgspcode==32234,"uncorrected",source_methodqualifier))

#look at what is left
temp <- dat %>% filter(parameter_name=="Chlorophyll a" & is.na(source_methodqualifier))
unique(temp$source_methodqualifier)
unique(temp$source_labmethod_name)
