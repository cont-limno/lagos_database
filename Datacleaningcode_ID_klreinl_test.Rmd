...
vars2process <- vars2process[!vars2process %in%Secchi_params]
vars2process

data_secchi<-data[data$CharacteristicName%in%Secchi_params,]

###create dataset to check for non numeric objects in dataset
temp<-data_secchi %>% filter(is.na(as.numeric(ResultMeasureValue))) %>% 
...
temp1 <- data_secchi %>% filter(!is.na(ResultMeasureValue))
temp2 <- data_secchi %>% filter(is.na(ResultMeasureValue)) %>% 
  filter(!is.na(DetectionQuantitationLimitMeasure.MeasureValue))
data_secchi <- rbind(temp1,temp2)
rm(temp1,temp2)

#Check non-numerics in detectionlimitvalue (if non numeric exist, code out)
unique(data_secchi$DetectionQuantitationLimitMeasure.MeasureValue[is.na(as.numeric(data_secchi$DetectionQuantitationLimitMeasure.MeasureValue))])

data_secchi <- data_secchi %>% mutate(DetectionQuantitationLimitMeasure.MeasureValue = as.numeric(DetectionQuantitationLimitMeasure.MeasureValue))

#generate Conversions
methods_table<-data_secchi %>% 
  select(CharacteristicName,
         ResultMeasure.MeasureUnitCode,
         Method_Id,
         ResultAnalyticalMethod.MethodName,
...
if(file.exists("conversion_files/secchi_Methods_Conversion.csv")==FALSE) {
  write_csv(methods_table,"conversion_files/secchi_Methods.csv")
  print("Conversion File needs to be created")
}

conversions <- read.csv(file = "conversion_files/secchi_Methods_Conversion.csv") # read in list of conversion factors
conversions[conversions == ""] <- NA #assign NA to blank values
check_methods<-merge(conversions,methods_table, by=c("CharacteristicName",
                                                     "ResultMeasure.MeasureUnitCode",
                                                     "DetectionQuantitationLimitMeasure.MeasureUnitCode",
...
data.size <- nrow(data_secchi) #check to make sure data frame stays same size

data_secchi<-merge(data_secchi,conversions,by=c("CharacteristicName",
                                                     "ResultMeasure.MeasureUnitCode",
                                                     "DetectionQuantitationLimitMeasure.MeasureUnitCode",
                                                     "Method_Id",
                                                     "ResultAnalyticalMethod.MethodName", 
                                                     "USGSPCode"),all.x = T) #Assign conversion factors using unique characteristic and method identifiers 
nrow(data_secchi) - data.size #should be zero!

data_secchi <- data_secchi %>% 
  mutate(Converted = ResultMeasureValue*Conversion) %>% 
  mutate(Converted_dl = DetectionQuantitationLimitMeasure.MeasureValue*Conversion_dl)

data_secchi_temp <- data_secchi %>% 
  filter(!is.na(ResultMeasureValue)) %>% 
  filter(is.na(Converted))%>% 
  select(CharacteristicName,
         ResultMeasure.MeasureUnitCode,
...
data_secchi_temp_notes <- conversions %>% 
  filter(!is.na(Notes))
...
rm(temp)
rm(data.size)
rm(data_secchi_temp_notes)

#assing LAGOS Units
data_secchi$ResultMeasure.MeasureUnitCode[!is.na(data_secchi$ResultMeasure.MeasureUnitCode)] <- "m"
data_secchi$DetectionQuantitationLimitMeasure.MeasureUnitCode[!is.na(data_secchi$DetectionQuantitationLimitMeasure.MeasureUnitCode)] <- "m"

#Assing Lagos Characteristic Name
...
Silica_params<-c("Silica", "Silicate", "Silicon") 
vars2process<-vars2process[! vars2process %in% Silica_params] #remove grab samples from list
vars2process #check if there are any missed variable

data_silica<-data[data$CharacteristicName%in%Silica_params,] #want to match all records to the vector we created above 
unique(data_silica$ResultSampleFractionText)
data_silica <- data_silica %>% filter(is.na(ResultSampleFractionText) | ResultSampleFractionText=="Dissolved")
unique(data_silica$ResultSampleFractionText)


###create dataset to check for non numeric objects in dataset
temp<-data_silica %>% filter(is.na(as.numeric(ResultMeasureValue)))  #checks for non numeric values in result measure value - if NA's not losing any other data - can change #391 NA's - can change
unique(temp$ResultMeasureValue)
...
#overwrite < with NA
data_silica <- data_silica %>% 
  mutate(ResultMeasureValue = ifelse(grepl("<",ResultMeasureValue,fixed=TRUE),NA,ResultMeasureValue))

#convert to numeric once done
data_silica<-data_silica %>% mutate(ResultMeasureValue = as.numeric(ResultMeasureValue)) # non numeric values become NA
#check detection limits
unique(data_silica$DetectionQuantitationLimitMeasure.MeasureValue[is.na(as.numeric(data_silica$DetectionQuantitationLimitMeasure.MeasureValue))])
#no non NA values, convert to numeric
data_silica<-data_silica %>% mutate(DetectionQuantitationLimitMeasure.MeasureValue = as.numeric(ResultMeasureValue)) # non numeric values become NA

methods_table<-data_silica %>% 
  select(CharacteristicName,
         ResultMeasure.MeasureUnitCode,
         Method_Id,
         ResultAnalyticalMethod.MethodName,
...
if(file.exists("conversion_files/Si_Methods_Conversion.csv")==FALSE) {
  write_csv(methods_table,"conversion_files/Si_Methods.csv")
  print("Conversion File needs to be created")
}

conversions <- read.csv(file = "conversion_files/Si_Methods_Conversion.csv") # read in list of conversion factors
conversions[conversions == ""] <- NA #assign NA to blank values
check_methods<-merge(conversions,methods_table, by=c("CharacteristicName",
                                                     "ResultMeasure.MeasureUnitCode",
                                                     "DetectionQuantitationLimitMeasure.MeasureUnitCode",
...
data.size <- nrow(data_silica) #check to ensure data frame stays same size after merge
data_silica<-merge(data_silica,conversions,by=c("CharacteristicName",
                                                     "ResultMeasure.MeasureUnitCode",
                                                     "DetectionQuantitationLimitMeasure.MeasureUnitCode",
                                                     "Method_Id",
                                                     "ResultAnalyticalMethod.MethodName", 
                                                     "USGSPCode"),all.x = T) #Assign conversion factors using unique characteristic and method identifiers 
nrow(data_silica) - data.size #should be zero!
...
data_silica_dl_temp <- data_silica %>% 
  filter(!is.na(DetectionQuantitationLimitMeasure.MeasureValue)) %>% 
  filter(is.na(Converted_dl)) %>% 
  select(CharacteristicName,
         ResultMeasure.MeasureUnitCode,
...
data_silica_temp_notes <- conversions %>% 
  filter(!is.na(Notes))

data_silica<- data_silica %>% filter(Conversion!=-1e6) %>% 
  filter(Conversion_dl != -1e6)
...
rm(temp)
rm(data.size)
rm(data_silica_temp_notes)
rm(data_silica_dl_temp)

data_silica$ResultMeasure.MeasureUnitCode[!is.na(data_silica$ResultMeasure.MeasureUnitCode)] <- "mg/l"
data_silica$DetectionQuantitationLimitMeasure.MeasureUnitCode[!is.na(data_silica$DetectionQuantitationLimitMeasure.MeasureUnitCode)] <- "mg/l"

unique(data_silica$ResultMeasure.MeasureUnitCode) #checked units #change to ug/l
unique(data_silica$DetectionQuantitationLimitMeasure.MeasureUnitCode) #checked units #all NA's no change needed

data_silica$CharacteristicName<-"Silica, dissolved"  #set param name
```
