...
variable <- "PH_FIELD"
data_file <- paste0("dat07_",variable)
dat07_chem1 <-
    dat07_chem %>%
    select("VISIT_NO",'SITE_ID','DATECHEM','SAMPLE_DEPTH',starts_with(variable),COMMENT_FLD_CHEM,COMMENT_LAB_CHEM,COMMENT_IM_CHEM,COM_FIELD_PH) %>% 
    select(-c(contains("_HT")))
dat07_chem1$SAMPLE_COMMENTS <- paste(dat07_chem1$COMMENT_FLD_CHEM,dat07_chem1$COMMENT_LAB_CHEM,dat07_chem1$COMMENT_IM_CHEM,dat07_chem1$COM_FIELD_PH,sep=' ')
names(dat07_chem1)[names(dat07_chem1) == variable] <- "SAMPLE_RESULT"
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('SPECIFIED')
dat07_chem1$SAMPLE_UNITS<-c('')
dat07_chem1$SAMPLE_MDL<-c('')
dat07_chem1$SAMPLE_FLAG<-''
dat07_chem1$SAMPLE_QA_FLAG<-c('')
dat07_chem1$SAMPLE_LAB_FLAG<-c('')
dat07_chem1$UID<-c('')
dat07_chem1$SAMPLE_DEPTH <- dat07_chem1$PH_FIELD_DEPTH
assign(data_file, dat07_chem1 %>% select(VISIT_NO,SITE_ID,DATECHEM,UID,starts_with("SAMPLE")))
...
dat07_chem1 <-
    ...
    select("VISIT_NO",'SITE_ID','DATECHEM','SAMPLE_DEPTH',starts_with(variable),COMMENT_FLD_CHEM,COMMENT_LAB_CHEM,COMMENT_IM_CHEM,COM_FIELD_PH) %>% 
    select(-c(contains("_HT")))
dat07_chem1$SAMPLE_COMMENTS <- paste(dat07_chem1$COMMENT_FLD_CHEM,dat07_chem1$COMMENT_LAB_CHEM,dat07_chem1$COMMENT_IM_CHEM,sep=' ')
names(dat07_chem1)[names(dat07_chem1) == variable] <- "SAMPLE_RESULT"
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
dat07_chem1$SAMPLE_UNITS<-c('uS/cm @ 25C')
flag<- paste0(variable,"_FLAG")
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c(2)
dat07_chem1$SAMPLE_FLAG<-dat07_chem[,flag]
dat07_chem1$SAMPLE_QA_FLAG<-c('')
dat07_chem1$SAMPLE_LAB_FLAG<-c('')
dat07_chem1$UID<-c('')
assign(data_file, dat07_chem1 %>% select(VISIT_NO,SITE_ID,DATECHEM,UID,starts_with("SAMPLE")))
...
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
dat07_chem1$SAMPLE_UNITS<-c('ueq/l')
flag<- paste0(variable,"_FLAG")
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c('')
dat07_chem1$SAMPLE_FLAG<-dat07_chem[,flag]
...
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
dat07_chem1$SAMPLE_UNITS<-c('NTU')
flag<- paste0(variable,"_FLAG")
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c(2)
dat07_chem1$SAMPLE_FLAG<-dat07_chem[,flag]
...
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
dat07_chem1$SAMPLE_UNITS<-c('ppm')
flag<- paste0(variable,"_FLAG")
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c(0.2)
dat07_chem1$SAMPLE_FLAG<-dat07_chem[,flag]
dat07_chem1$SAMPLE_QA_FLAG<-c('')
dat07_chem1$SAMPLE_LAB_FLAG<-c('')
dat07_chem1$UID<-c('')
...
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
dat07_chem1$SAMPLE_UNITS<-c('PCU')
flag<- paste0(variable,"_FLAG")
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c(5)
dat07_chem1$SAMPLE_FLAG<-dat07_chem[,flag]
dat07_chem1$SAMPLE_QA_FLAG<-c('')
dat07_chem1$SAMPLE_LAB_FLAG<-c('')
dat07_chem1$UID<-c('')
...
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
dat07_chem1$SAMPLE_UNITS<-c('mg/l')
flag<- paste0(variable,"_FLAG")
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c(0.1)
dat07_chem1$SAMPLE_FLAG<-dat07_chem[,flag]
...
dat07_chem1 <-
    dat07_chem %>%
    select("VISIT_NO",'SITE_ID','DATECHLA','SAMPLE_DEPTH',starts_with(variable),'FLAG_FLD_CHLA','FLAG_LAB_CHLA','COMMENT_FLD_CHLA','COMMENT_LAB_CHLA',COMMENT_FLD_CHEM,COMMENT_LAB_CHEM,COMMENT_IM_CHEM) %>% 
    select(-c(contains("_HT")))#%>% 
dat07_chem1$SAMPLE_COMMENTS <- paste(dat07_chem1$COMMENT_FLD_CHEM,dat07_chem1$COMMENT_LAB_CHEM,dat07_chem1$COMMENT_IM_CHEM,dat07_chem1$COMMENT_FLD_CHLA,dat07_chem1$COMMENT_LAB_CHLA,sep=' ')
# select(-K) %>% rename("K"="K_PPM")
names(dat07_chem1)[names(dat07_chem1) == variable] <- "SAMPLE_RESULT"
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
...
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c('')
dat07_chem1$SAMPLE_FLAG<-c('')#dat07_chem[,flag]
dat07_chem1$SAMPLE_QA_FLAG<-c('')
dat07_chem1$SAMPLE_LAB_FLAG<-c('')
dat07_chem1$UID<-c('')
dat07_chem1 <- dat07_chem1 %>% rename("DATECHEM" = "DATECHLA")
assign(data_file, dat07_chem1 %>% select(VISIT_NO,SITE_ID,DATECHEM,UID,starts_with("SAMPLE")))
...
dat07_chem1 <-
    dat07_chem %>%
    select("VISIT_NO",'SITE_ID','DATESECCHI','SAMPLE_DEPTH',starts_with(variable),COMMENT_FLD_CHEM,COMMENT_LAB_CHEM,COMMENT_IM_CHEM,COMMENT_SECCHI,CLEAR_TO_BOTTOM) %>% 
    select(-c(contains("_HT")))#%>% 
dat07_chem1$SAMPLE_COMMENTS <- paste(dat07_chem1$COMMENT_FLD_CHEM,dat07_chem1$COMMENT_LAB_CHEM,dat07_chem1$COMMENT_IM_CHEM,dat07_chem1$COMMENT_SECCHI,sep=' ')
# select(-K) %>% rename("K"="K_PPM")
names(dat07_chem1)[names(dat07_chem1) == variable] <- "SAMPLE_RESULT"
dat07_chem1$SAMPLE_NAME<-c(variable)
dat07_chem1$SAMPLE_TYPE<-c('INTEGRATED')
dat07_chem1$SAMPLE_UNITS<-c('m')
flag<- paste0(variable,"_FLAG")
dat07_chem1 %>% select(contains('_FLAG')) %>% distinct()
dat07_chem1$SAMPLE_MDL<-c('')
dat07_chem1$SAMPLE_FLAG<-dat07_chem1$CLEAR_TO_BOTTOM
dat07_chem1$SAMPLE_QA_FLAG<-c('')
dat07_chem1$SAMPLE_LAB_FLAG<-c('')
dat07_chem1$UID<-c('')
dat07_chem1 <- dat07_chem1 %>% rename("DATECHEM" = "DATESECCHI")
assign(data_file, dat07_chem1 %>% select(VISIT_NO,SITE_ID,DATECHEM,UID,starts_with("SAMPLE")))

rm(dat07_chem)
rm(dat07_chem1)

Discrete_data07<- do.call("rbind", lapply(ls(pattern='dat07_'),get))
...
Discrete_data07 <- Discrete_data07[, c('UID','SAMPLE_UNITS','SAMPLE_RESULT','SAMPLE_FLAG','SAMPLE_QA_FLAG','SAMPLE_LAB_FLAG',
                                    'SAMPLE_TYPE','SAMPLE_MDL','SAMPLE_NAME','SAMPLE_DEPTH','SITE_ID','VISIT_NO')]
...
dat07_site<-
    dat07_siteinfo %>%
    select(c('SITE_ID',"VISIT_NO",'DATE_COL','FLD_LAT_DD','FLD_LON_DD','ST',
             'CNTYNAME','NHDNAME','LAKENAME'))


all_data07_complete<-merge(dat07_site,Discrete_data07,by=c('SITE_ID','VISIT_NO'),keep=T)
all_data07_complete$NAME3<-c('')

rm(list=setdiff(ls(), "all_data07_complete"))
...
all_data12<-rbind(dat12_secchi,Discrete_data12)
...
all_data12_complete<-merge(all_data12_complete,dat12_profile,by='UID',keep=T)

rm(list=setdiff(ls(), c("all_data07_complete","all_data12_complete")))
...
dat17_profile<-read.csv("./NLA/nla_2017_profile-data.csv")%>% 
    mutate(INDEX_LAT_DD = as.numeric(INDEX_LAT_DD),
           INDEX_LON_DD = as.numeric(INDEX_LON_DD)) %>% 
    drop_na(INDEX_LAT_DD) %>% 
    select(UID,INDEX_LAT_DD,INDEX_LON_DD)
...
all_data17<-rbind(Discrete_data17)
...
all_data17_complete<-merge(dat17_profile,all_data17_complete,by='UID',keep=T)

rm(list=setdiff(ls(), c("all_data07_complete","all_data12_complete","all_data17_complete")))
...
all_data07_complete1 <- all_data07_complete[, c('UID','SITE_ID',"VISIT_NO",'DATE_COL','FLD_LAT_DD','FLD_LON_DD','ST',
...
all_data12_complete1 <- all_data12_complete[, c('UID','SITE_ID',"VISIT_NO",'DATE_COL','INDEX_LAT_DD','INDEX_LON_DD','STATE',
...
all_data17_complete1 <- all_data17_complete[, c('UID','SITE_ID',"VISIT_NO",'DATE_COL','INDEX_LAT_DD','INDEX_LON_DD','STATE_NM',
                                               'CNTYNAME','GNIS_ID','GNIS_NAME','NARS_NAME','SAMPLE_UNITS',
                                               'SAMPLE_RESULTS','SAMPLE_FLAG1','SAMPLE_FLAG2','SAMPLE_FLAG3',
                                               'SAMPLE_TYPE','SAMPLE_MDL','SAMPLE_NAME','DEPTH')]
