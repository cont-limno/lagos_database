...
COdatPull<-read.csv("~/Lagos Limno/CO_alldata_1975_to_2020.csv")
...
data<- COdatPull[COdatPull$CharacteristicName %notin% remove_params$Unique.parameters,]
...
fractionkeep<-c("Dissolved")
data_CO<-data_CO[which(data_CO$ResultSampleFractionText %in% fractionkeep | is.na(data_CO$ResultSampleFractionText)),] #keeping if it's ones we chose or NA
unique(data_CO$ResultSampleFractionText)

###create dataset to check for non numeric objects in dataset
temp<-data_CO %>% filter(is.na(as.numeric(ResultMeasureValue)))  #checks for non numeric values in result measure value - if NA's not losing any other data - can change
data_CO<-data_CO %>% mutate(ResultMeasureValue = as.numeric(ResultMeasureValue)) # non numeric values become NA

unique(data_CO$ResultMeasure.MeasureUnitCode) #checked units
temp<-data_CO[which(!is.na(data_CO$DetectionQuantitationLimitMeasure.MeasureUnitCode) & data_CO$ResultMeasure.MeasureUnitCode != data_CO$DetectionQuantitationLimitMeasure.MeasureUnitCode),] #checks to see if detection limit units same as obvs units



#comeback to # undecided what to do with units "uS/cm"   "ug/l"    "umho/cm"
...
###create dataset to check for non numeric objects in dataset
temp<-data_NH4 %>% filter(is.na(as.numeric(ResultMeasureValue)))  #checks for non numeric values in result measure value - if NA's not losing any other data - can change
data_NH4<-data_NH4 %>% mutate(ResultMeasureValue = as.numeric(ResultMeasureValue)) # non numeric values become NA

unique(data_NH4$ResultMeasure.MeasureUnitCode) #checked units
temp<-data_NH4[which(!is.na(data_NH4$DetectionQuantitationLimitMeasure.MeasureUnitCode) & data_NH4$ResultMeasure.MeasureUnitCode != data_NH4$DetectionQuantitationLimitMeasure.MeasureUnitCode),] #checks to see if detection limit units same as obvs units

#WORKING WITH Ammonia and ammonium --- all the other 3 species have 0 observations

temp<-data_NH4 %>% filter(CharacteristicName == "Ammonia and ammonium") %>% filter(is.na(ResultMeasure.MeasureUnitCode)) %>% filter(!is.na(ResultMeasureValue))


temp<-data_NH4 %>% filter(CharacteristicName == "Ammonia and ammonium")
unique(temp$ResultMeasure.MeasureUnitCode)
unique(temp$DetectionQuantitationLimitMeasure.MeasureUnitCode)

#We have mg/l as N and mg/l NH4 - Need to make conversion for both into ug/L as N

data_NH4<-resultconvert(dat = data_NH4, variable = "Ammonia and ammonium", unit = "mg/l as N", out_unit = "ug/l_converted", conv_factor = 1000)
data_NH4<-detectconvert(dat = data_NH4, variable = "Ammonia and ammonium", unit = "mg/l as N", out_unit = "ug/l_converted", conv_factor = 1000)
...
data_NH4<-resultconvert(dat = data_NH4, variable = "Ammonia and ammonium", unit = "mg/l NH4", out_unit = "ug/l_converted", conv_factor = 0.7765*1000)
data_NH4<-detectconvert(dat = data_NH4, variable = "Ammonia and ammonium", unit = "mg/l NH4", out_unit = "ug/l_converted", conv_factor = 0.7765*1000)
...
Alk_params<-c("Alkalinity, total" , "Alkalinity, carbonate", "Alkalinity") #only values for Alk
data_alk<-data[data$CharacteristicName%in%Alk_params,] #want to match all records to the vector we created above which have TKN
unique(data_alk$ResultSampleFractionText)
temp<-data_TKN %>% filter(is.na(ResultSampleFractionText)) #to check NA's or anything unclear - we check and decide what to keep
fractionkeep<-c("Total", "Dissolved")
data_alk<-data_alk[which(data_alk$ResultSampleFractionText %in% fractionkeep | is.na(data_alk$ResultSampleFractionText)),] #keeping if it's ones we chose or NA
unique(data_alk$ResultSampleFractionText)
...
unique(temp$ResultMeasureValue)
data_alk$MeasureQualifierCode[which(data_alk$ResultMeasureValue == "ND")]<-data_alk$ResultMeasureValue[which(data_alk$ResultMeasureValue == "ND")]
data_alk<-data_alk %>% filter(ResultMeasureValue != "NT" | is.na(ResultMeasureValue))
temp<-data_alk %>% filter(is.na(as.numeric(ResultMeasureValue)))  #checks for non numeric values in result measure value - if NA's not losing any other data - can change
data_alk<-data_alk %>% mutate(ResultMeasureValue = as.numeric(ResultMeasureValue)) # non numeric values become NA


temp<-data_alk %>% filter(CharacteristicName == "Alkalinity, carbonate") #checked to see if carbonate+bicarbonate ions - its not - just alkalinity - KEEP



unique(data_alk$ResultMeasure.MeasureUnitCode) #checked units - got mg/l, ug/l, mg/l caco3 - need to check species for mg/l and ug/l - looking at methods
temp<-data_alk %>% filter(ResultMeasure.MeasureUnitCode == "mg/l")
unique(temp$ResultAnalyticalMethod.MethodIdentifier) #310.1, #2320 - B
unique(temp$ResultAnalyticalMethod.MethodName)
temp<-data_alk %>% filter(ResultMeasure.MeasureUnitCode == "ug/l")

temp<-data_alk[which(!is.na(data_alk$DetectionQuantitationLimitMeasure.MeasureUnitCode) & data_alk$ResultMeasure.MeasureUnitCode != data_alk$DetectionQuantitationLimitMeasure.MeasureUnitCode),] #checks to see if detection limit units same as obvs units - got 0, all the same
